...
function estimarCusto(mensagens: number | null) {
  ...
  if (mensagens == null || !preco) {
    return {
      ...
      porMensagemUSD: null as number | null,
      estimado: true,
      nota: !preco
  ...
  return {
    modelo,
    totalUSD: porMsg * mensagens,
    porMensagemUSD: porMsg,
    estimado: true,
...
export async function getMetrics(): Promise<Metrics> {
  const fontes = {
    ...
    n8n: !!(process.env.N8N_API_KEY && process.env.N8N_WORKFLOW_ID),
  };

  // HubSpot (métricas 2 e 3) com fallback no seed
  let hub = {
    conversasUnicas: SEED.conversasUnicas,
    escaladas: SEED.escaladas,
    topicos: SEED.topicos,
    motivosEscalacao: SEED.motivosEscalacao,
    escalacoesPorDia: SEED.escalacoesPorDia,
  };
  if (fontes.hubspot) {
    try {
      hub = await getHubspotMetrics();
    } catch (e) {
      console.error("HubSpot falhou, usando seed:", e);
    }
  }

  // n8n (métrica 1 - mensagens)
  let mensagens: number | null = SEED.mensagens;
  if (fontes.n8n) {
    try {
      mensagens = await getN8nMensagens();
    } catch (e) {
      console.error("n8n falhou:", e);
    }
  }

  const taxaEscalacao =
    hub.conversasUnicas > 0 ? hub.escaladas / hub.conversasUnicas : null;
